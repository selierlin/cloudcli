...
function appendCompactionRow(
  ...
  // Fork tool-result messages carry structured (non-string) content; only
  // string content participates in compaction folding.
  ...
  if (msg.isCompactSummary) {
    ...
    for (let index = converted.length - 1; index >= 0; index -= 1) {
      ...
      if (row.type === 'assistant' && !row.compact && typeof row.content === 'string' && row.content.trim() === text) {
