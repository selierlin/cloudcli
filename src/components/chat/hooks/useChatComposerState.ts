...
export function useChatComposerState({
   ...
   * allocates the session via an async `POST /api/providers/sessions`. During
   * that round-trip the submit button stays enabled (nothing has set the
   * processing state yet), so a second Enter/click can run the same branch and
   * mint a second, identical session. The flag closes that window: a second
   * submit is dropped while one session-creation request is already in flight.
   */
