...
const getScopeLabel = (
  ...
  if (scope === 'user') {
    return mode === 'global' ? t('mcpForm.scope.userAllProviders') : t('mcpForm.scope.userGlobal');
  }
  ...
  return mode === 'global' ? t('mcpForm.scope.projectAllProviders') : t('mcpForm.scope.projectProvider');
};
...
const getScopeDescription = (
  ...
  if (scope === 'user') {
    return mode === 'global'
      ? t('mcpForm.scope.userAllProvidersDescription')
      : t('mcpForm.scope.userProviderDescription');
  }
  ...
  return mode === 'global'
    ? t('mcpForm.scope.projectAllProvidersDescription')
    : t('mcpForm.scope.projectProviderDescription');
};
...
export default function McpServerFormModal({
  ...
  const addButtonLabel = submitLabel ?? t('mcpForm.actions.addServerTo', { provider: providerName });
      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg border border-border bg-background">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-4">
          ...
          {isEditing && (
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-900/50">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">{getScopeLabel(formData.scope, mode, provider, t)}</span>
                  ...
                  {availableScopes.map((scope) => (
                    ...
                    >
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span>{getScopeLabel(scope, mode, provider, t)}</span>
                      </div>
                    </button>
                  ))}
                </div>
                <p className="mt-2 text-xs text-muted-foreground">{getScopeDescription(formData.scope, mode, provider, t)}</p>
              </div>
          ...
          {formData.importMode === 'form' && formData.transport === 'stdio' && (
            <div className="space-y-4">
              ...
              {supportsWorkingDirectory && (
                <div>
                  <label className="mb-2 block text-sm font-medium text-foreground">
                    {t('mcpForm.fields.workingDirectory')}
                  </label>
                  <Input
                    value={formData.cwd}
          ...
          {showCodexOnlyFields && formData.importMode === 'form' && formData.transport === 'http' && (
            <div>
              <label className="mb-2 block text-sm font-medium text-foreground">
                {t('mcpForm.fields.bearerTokenEnvVar')}
              </label>
              <Input
