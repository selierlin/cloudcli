...
/**
 * Reads inline image data from the SDK 0.150 response-item shape. The text for
 * the same turn arrives separately as an `item_completed` UserMessage event.
 */
...
export function parseCodexExecScript(input: unknown): CodexExecOperation[] {
  ...
  if (!source.includes('tools.')) {
    try {
      ...
      return command ? [{ kind: 'shell', command }] : [];
    } catch {
      // Not a legacy direct shell wrapper.
    }
    return [];
  }
...
async function getCodexSessionMessages(sessionId: string): Promise<CodexHistoryResult> {
  ...
  for await (const line of rl) {
    ...
    if (entry.type === 'event_msg') {
      ...
      // SDK 0.150 represents this lifecycle event differently from the older
      // `sub_agent_activity` envelope above.
      if (payload.type === 'item_completed' && payload.item?.type === 'SubAgentActivity'
        && payload.item?.kind === 'started') {
        const callId = readNonEmptyString(payload.item.id);
        const agentPath = readNonEmptyString(payload.item.agent_path);
    ...
    if (payload.type === 'message' && payload.role === 'user') {
      const metadata = readObjectRecord(payload.internal_chat_message_metadata_passthrough);
      const turnId = readNonEmptyString(metadata?.turn_id);
      const images = extractCodexInlineImages(payload.content);
      if (turnId && images) {
  ...
  async fetchHistory(
    ...
    for (const raw of result.messages) {
      for (const message of this.normalizeHistoryEntry(raw, sessionId)) {
        // SDK history can retain lifecycle snapshots for one native item.
        // Mirror realtime behavior by replacing the previous tool row.
        if (raw.type === 'item' && message.kind === 'tool_use') {
          const key = `${message.id}\u0000${message.kind}`;
          const index = historyMessageIndexes.get(key);
          if (index === undefined) {
            historyMessageIndexes.set(key, normalized.length);
            normalized.push(message);
          } else {
            normalized[index] = { ...normalized[index], ...message };
          }
        } else {
          normalized.push(message);
        }
