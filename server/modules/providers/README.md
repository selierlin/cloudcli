...
| DSH | `~/.dsh/skills`, `~/.agents/skills` | nearest git root `.dsh/skills` and `.agents/skills` | `/` | Mirrors the DSH skill plugin's default roots in rank order. Listing stays read-only because the harness owns loading. |
