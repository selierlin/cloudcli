...
> **承接**：`docs/research/工具内容换行与折行渲染评审优化方案.md`（L0–L6 已实施并验证）。
...
| 10 | `docs/research/工具内容换行与折行渲染评审优化方案.md` | §11.6（新增） | 记录本轮与 L0–L6 的关系（G2 补 L0 的同源漏点、G3 是 L0/L5 的全局根因） | 全部 |
...
| 11 | 本文件 + `工具内容换行与折行渲染评审优化方案.md` | §11.6 / §12 | 记录本轮与 L0–L6 的关系 |
...
> ⑥ §6.2 的断言落点（D4）是否合适。
>
> 若发现问题，请给出**可复现步骤**而非结论性描述——上一轮的教训是「只给结论的批注」需要牵头逐条反查，成本高。相关基线：本方案承接 `工具内容换行与折行渲染评审优化方案.md`（L0–L6 已实施）。
